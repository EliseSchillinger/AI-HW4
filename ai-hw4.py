...
def valid_move(pengu, current_test_location):
    ...
    if pengu.location != current_test_location and (pengu.can_move == False and pengu.valid_move == False and pengu.death == False):
        return True
    elif pengu.score >= 20 and pengu.death == True: # pengu can die on last move as long as he gets necessary score
        return True
...
def heuristic(pengu, path):
    """ adds to the priority queue frontier based on the number of fish remaining. 
        less fish remaining is better because the score is higher """
    add_to_frontier = ()
    add_to_frontier = (pengu.total_fish, path)

    return add_to_frontier

def main_greedy_bfs():
    """ main pengu loop: uses greedy best-first search algorithm to collect fish """
    ...
    while frontier: # as long as the frontier priority queue isn't empty, test algorithm
        ...
        if pengu_obj.score >= 20: # if pengu finds goal state: write to output and exit algorithm loop
        ...
        for move in range(1,10): # test all keypad moves excluding 5
            ...
            if valid_move(pengu_obj, current_test_location): # add to the frontier
                ...
                add_to_frontier = heuristic(pengu_obj,path)
                if frontier == []:
                    heapq.heappush(frontier, add_to_frontier) # push onto priority queue: maintains invariant
                else:
                    comparison_path = frontier[0]
                    if add_to_frontier[0] < comparison_path[0]: # if the path reduces number of remaining fish push to heap (becomes smallest value)
                        heapq.heappush(frontier, add_to_frontier)
                    else: # trying to prevent getting stuck on infinite loop: just adds to the end of frontier
